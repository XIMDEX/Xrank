	...
	var vote = function (publication, val) {
		publication.vote({val: parseInt(val), voter: req.cookies.xrank_id || xrank_id}, function(err){
			if(err) {
				...
				res.jsonp({notice: "Yout vote has been counted"});
			}
		});
	};
	...
	if(url) {
		return Publication.find({url:url}, function (err, publications) {
		  if (publications.length) {
		    ...
		    return res.jsonp({notice:"The publication is not in the database"});
		  }
		});
	} else {
		return res.send(400);	
...
//Hacking untrusted certificates
exports.cert = function(req, res) {
    var url = req.query.origin_url;
    res.redirect(url+'?cert_accepted=true');
}

exports.test = function(req, res) {
  res.send(209, 'Test response');
};